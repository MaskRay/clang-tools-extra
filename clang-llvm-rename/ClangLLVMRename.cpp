...
class RenameCallback : public MatchFinder::MatchCallback {
  DenseMap<const FunctionDecl *, bool> Map;
public:
  RenameCallback(
  ...
  void run(const MatchFinder::MatchResult &Result) override {
    ...
    //  auto I = Map.try_emplace(D, false);
    //  if (!I.second && !I.first->second)
    //    return;
    //  if (auto *MD = dyn_cast<CXXConstructorDecl>(D))
    //    return;
    //  if (auto *MD = dyn_cast<CXXDestructorDecl>(D))
    //    return;

    //  const DeclContext *SemDC = D->getDeclContext()->getRedeclContext();
    //  {
    //    const NamespaceDecl *ND;
    //    while ((ND = dyn_cast<NamespaceDecl>(cast<Decl>(SemDC)))) {
    //      if (ND->getName() == "llvm" || ND->getName() == "clang" || ND->getName() == "rapidjson")
    //        return;
  ...
  void registerMatchers(MatchFinder &M) {
    //M.addMatcher(functionDecl().bind("FuncDecl"), this);
...
private:
  ...
  void convert(SourceManager &SM, SourceLocation Loc, StringRef MustMatch) {
    ...
    if (!SM.isInMainFile(SpellingLoc))
      return;
    if (SpellingLoc.getRawEncoding() >= 7) {
      if (std::string(SM.getCharacterData(SpellingLoc.getLocWithOffset(-7)), 7) == "struct ")
        return;
      if (std::string(SM.getCharacterData(SpellingLoc.getLocWithOffset(-6)), 6) == "class ")
        return;
    }
    ...
    if (!Repl.getFilePath().empty())
      consumeError(FileToReplacements[Repl.getFilePath()].add(Repl));
  }
  ...
  std::string tolower(StringRef S) {
    ...
    for (StringRef Prefix : {"CI", "ARM", "PPC", "RISCV", "LMA", "VMA", "ELF",
...
int main(int argc, const char **argv) {
  ...
  tooling::CommonOptionsParser OptParser(argc, argv, ToolTemplateCategory);
